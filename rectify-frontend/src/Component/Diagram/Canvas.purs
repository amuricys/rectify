...
handleQuery :: forall m a. MonadEffect m => Query a -> H.HalogenM (Diagram.State Unit) Diagram.Action () Void m (Maybe a)
handleQuery = case _ of
  ...
  ReceiveDiagramData msg a -> do
    x <- H.get
    case x.diagram of
      Just d -> Diagram.updateDiagram d msg a
      -- If for some reason we don't have a diagram, we'll initialize a surface one
      Nothing -> Diagram.initDiagram canvasDivId msg Diagram.Surface.diag *> pure (Just a)
