...
import Went.GraphObject (MadeGraphObject(..)) 
...
import Went.Template.Makers (Made(..), MadeNode)
...
addNodeTemplate
  ...
  => IsDiagram d
  => String
  -> MadeGraphObject nodeData b b
  -> MakeDiagram nodeData linkData d Unit
addNodeTemplate templateName (MadeGraphObject howToMadeNode) = MakeDiagram $ do
  handle <- ask
...
addLinkTemplate
  :: forall nodeData linkData d
   . IsDiagram d
  => String
  -> MadeGraphObject linkData Link_ Link_
  -> MakeDiagram nodeData linkData d Unit
addLinkTemplate templateName (MadeGraphObject howToMakeLinkemplate) = MakeDiagram $ do
  handle <- ask
...
addGroupTemplate
  ...
  -> MadeGraphObject nodeData b b
  -> MakeDiagram nodeData linkData d Unit
addGroupTemplate templateName (MadeGraphObject howToMadeGroup) = MakeDiagram $ do
  handle <- ask
...
make :: forall nodeData linkData. String -> MakeDiagram nodeData linkData Diagram_ Unit -> Effect Diagram_
make div (MakeDiagram m) = do
  diag <- newDiagram div
  runReaderT m diag
  pure diag

makePalette :: forall nodeData linkData. String -> MakeDiagram nodeData linkData Palette_ Unit -> Effect Palette_
