...
import Servant ((:>), Server, serve, Context (EmptyContext))
...
main :: IO ()
main = do
  putStrLn "Server running on port 9160"
  run 9160 $ app
  where
...
wsApp pending = liftIO $ do
  ...
  -- _ <- forkIO $ finally (serverSendMessage conn stateVar) (putStrLn "Client disconnected")
