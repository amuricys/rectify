...
instance ToJSON (FinnRandoSolution n) where
  ...
  toJSON (FinnRandoSolution v) = 
    ...
    where vals = fromList [("cities", V.toList . fmap (tupleToPoint . bimap ad ad . cityWGS84) $ v)]
          tupleToPoint (x, y) = Object . fromList $ [("x", toJSON x), ("y", toJSON y)]
          ad x = (x - 70) * 50.0
...
isingAcceptance
    :: Double -- ^ energyState
    -> Double -- ^ energyNeighbor
    -> Double   -- ^ temperature
    -> Probability
isingAcceptance hu hv beta
